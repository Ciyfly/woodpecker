 ...
 * @LastEditors: recar
 * @LastEditTime: 2022-04-29 18:02:03
 */
package xray
...
func LoadXrayYamlPoc(pocNames []string) []*Poc {
	...
	if len(pocNames) != 0 {
		for _, name := range pocNames {
			xrayPocPath := path.Join(pocDIr, name+".yml")
			if util.FileIsExist(xrayPocPath) {
				pocPathList = append(pocPathList, xrayPocPath)
			}
		}
	} else {
...
// db poc to yaml poc
func DbPoc2XrayPoc(dbPocs []db.Poc) []*Poc {
	var xrayPocs []*Poc
